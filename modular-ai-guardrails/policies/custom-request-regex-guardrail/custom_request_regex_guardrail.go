...
const policyName = "custom-request-regex-guardrail"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=messagesText(v);if containsAny(text,`(?i)\b(?:jailbreak|DAN|developer\s+mode)\b`,`(?i)cross[-\s]?(?:tenant|patient)`,`(?i)forge(?:d)?\s+(?:approval|authorization)`){setFinding(req,"REQUEST_POLICY_PATTERN","Request matched a blocked security pattern.",nil)}
    return nil
}
