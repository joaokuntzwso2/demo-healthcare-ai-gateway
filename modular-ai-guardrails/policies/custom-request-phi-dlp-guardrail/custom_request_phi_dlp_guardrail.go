package customrequestphidlpguardrail
...
const policyName = "custom-request-phi-dlp-guardrail"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=messagesText(v);if containsAny(text,`(?i)\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b`,`\b\d{3}-\d{2}-\d{4}\b`,`(?i)\b(?:sk-[A-Za-z0-9_-]{12,}|Bearer\s+[A-Za-z0-9._-]{12,})\b`){setFinding(req,"PHI_PII_SECRET_DETECTED","Direct identifier or secret detected in model-bound request. Use server-side references and pseudonyms.",nil)}
    return nil
}
