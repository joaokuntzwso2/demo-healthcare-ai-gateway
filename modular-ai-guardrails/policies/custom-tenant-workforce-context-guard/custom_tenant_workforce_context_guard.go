...
const policyName = "custom-tenant-workforce-context-guard"

const pendingKey = "wso2.ai.pending-request-block"
const canonicalKey = "helios.canonical-text"

type Policy struct{ expectedApp string }
...
func GetPolicy(_ policy.PolicyMetadata, params map[string]interface{}) (policy.Policy, error) {
	expected, _ := params["expectedApp"].(string)
	return &Policy{expectedApp: expected}, nil
}
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
	...
	c := contextMap(req)
	if c == nil || str(c, "tenant") == "" || str(c, "actor") == "" || str(c, "role") == "" || str(c, "app") == "" {
		setFinding(req, "CLINICAL_DATA_NOT_AUTHORIZED", "Signed workforce/application context is missing.", nil)
		return nil
	}
