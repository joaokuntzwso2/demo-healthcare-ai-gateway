package custompatientencounterbindingguard
...
const policyName = "custom-patient-encounter-binding-guard"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
	...
	text := messagesText(v)
	patient := str(c, "patientId")
	enc := str(c, "encounter")
	for _, id := range candidateIDs(text, "pat-") {
		if !strings.EqualFold(id, patient) {
			setFinding(req, "PATIENT_SCOPE_MISMATCH", "Request references a patient outside the signed patient context.", map[string]interface{}{"requested": id})
			break
		}
	}
	for _, id := range candidateIDs(text, "enc-") {
		if enc != "" && !strings.EqualFold(id, enc) {
			setFinding(req, "PATIENT_SCOPE_MISMATCH", "Request references an encounter outside the signed encounter context.", map[string]interface{}{"requestedEncounter": id})
			break
		}
	}
	return nil
