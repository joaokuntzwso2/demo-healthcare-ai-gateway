...
const policyName = "canonicalize-and-classify"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    raw:=messagesText(v);canon:=canonicalize(raw);ensureMetadata(req);req.Metadata[canonicalKey]=canon;if canon!=raw{req.Metadata["helios.encoded-content-detected"]=true}
    return nil
}
