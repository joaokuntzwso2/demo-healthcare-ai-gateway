package customjailbreakauthoritybypassguardrail
...
const policyName = "custom-jailbreak-authority-bypass-guardrail"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=messagesText(v);if req.Metadata!=nil{if s,ok:=req.Metadata[canonicalKey].(string);ok{text=s}};if containsAny(text,`(?i)ignore\s+(?:all\s+)?(?:prior|previous)\s+instructions`,`(?i)bypass\s+(?:the\s+)?(?:guardrail|authorization|clinical\s+safety|policy)`,`(?i)unrestricted\s+(?:doctor|clinician|administrator)`,`(?i)reveal\s+(?:the\s+)?system\s+prompt`){setFinding(req,"JAILBREAK_OR_AUTHORITY_BYPASS","Prompt attempts to override authority or system policy.",nil)}
    return nil
}
