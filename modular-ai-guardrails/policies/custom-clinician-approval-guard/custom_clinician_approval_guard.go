...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=strings.ToLower(messagesText(v));if containsAny(text,`\b(?:automatically|autonomously|without\s+(?:human|clinician)\s+approval)\b.{0,60}\b(?:order|prescrib|test|medication)`,`\bforge(?:d)?\s+(?:clinician\s+)?approval\b`,`\bsubmit_for_clinician_approval\b`){setFinding(req,"CLINICIAN_APPROVAL_REQUIRED","The model cannot create or forge clinician approval; approval is an external bound workflow.",nil)}
    return nil
}
