package customstructuredclinicaloutputguard
import (
    "context"
    "encoding/json"
    "fmt"
...
const policyName = "custom-structured-clinical-output-guard"
...
func (p *Policy) OnResponseBody(_ context.Context, resp *policy.ResponseContext, _ map[string]interface{}) policy.ResponseAction {text,err:=assistantText(resp);if err!=nil{if text==""{return nil};return blockResponse("RESPONSE_STRUCTURE_INVALID",err.Error(),nil)};trim:=strings.TrimSpace(text);if strings.HasPrefix(trim,"{"){var obj map[string]interface{};if json.Unmarshal([]byte(trim),&obj)==nil{if _,clinical:=obj["clinicalAction"];clinical{missing:=[]string{};for _,k:=range []string{"decision","evidence","reasonCodes"}{if _,ok:=obj[k];!ok{missing=append(missing,k)}};if len(missing)>0{return blockResponse("STRUCTURED_CLINICAL_OUTPUT_INVALID","Structured clinical action output is missing required evidence fields.",map[string]interface{}{"missing":missing})}}}};return policy.DownstreamResponseModifications{}}
