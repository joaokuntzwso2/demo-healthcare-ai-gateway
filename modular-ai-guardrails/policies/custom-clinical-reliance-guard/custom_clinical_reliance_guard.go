package customclinicalrelianceguard
import (
    "context"
    "encoding/json"
    "fmt"
...
const policyName = "custom-clinical-reliance-guard"
...
func (p *Policy) OnResponseBody(_ context.Context, resp *policy.ResponseContext, _ map[string]interface{}) policy.ResponseAction {text,err:=assistantText(resp);if err!=nil{if text==""{return nil};return blockResponse("RESPONSE_STRUCTURE_INVALID",err.Error(),nil)};if containsAny(text,`(?i)\bpatient\s+(?:definitely|certainly)\s+has\b`,`(?i)\bdiagnosis\s+is\s+(?:confirmed|certain)\b`,`(?i)\bno\s+clinician\s+review\s+(?:is\s+)?needed\b`,`(?i)\bguaranteed\s+(?:safe|effective)\b`){return blockResponse("UNSUPPORTED_CLINICAL_CERTAINTY","Response overstates diagnostic certainty, safety, or clinical reliance beyond evidence.",nil)};return policy.DownstreamResponseModifications{}}
