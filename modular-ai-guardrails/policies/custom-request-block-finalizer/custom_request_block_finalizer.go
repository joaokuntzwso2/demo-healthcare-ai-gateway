...
const policyName = "custom-request-block-finalizer"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
 if req==nil||req.Metadata==nil{return nil}; raw,ok:=req.Metadata[pendingKey];if !ok{return nil}; finding,_:=raw.(map[string]interface{}); code:=str(finding,"reasonCode");reason:=str(finding,"reason"); if code==""{code="REQUEST_BLOCKED"}; if reason==""{reason="Request denied by Helios clinical AI security policy chain."}; body,_:=json.Marshal(map[string]interface{}{"type":"HELIOS_CLINICAL_AI_GUARDRAIL","message":map[string]interface{}{"action":"GUARDRAIL_INTERVENED","direction":"REQUEST","interveningGuardrail":textParam(finding,"policy",policyName),"reasonCode":code,"actionReason":reason}});return policy.ImmediateResponse{StatusCode:422,Headers:map[string]string{"Content-Type":"application/json","Cache-Control":"no-store"},Body:body,AnalyticsMetadata:map[string]any{"reasonCode":code,"heliosBlocked":true}}
}
