package custompurposescopeguard
...
const policyName = "custom-purpose-scope-guard"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
	...
	if purpose == "appointment-scheduling" && containsAny(text, `\b(?:diagnos|lab|allerg|medication|clinical\s+note|entire\s+chart|raw\s+chart)`) {
		setFinding(req, "PURPOSE_SCOPE_EXCEEDED", "Appointment purpose cannot release unrelated chart categories.", nil)
	}
	if purpose == "medication-review" && !(hasScope(c, "medications:read") || hasScope(c, "allergies:read")) {
		setFinding(req, "CLINICAL_DATA_NOT_AUTHORIZED", "Medication-review purpose lacks required clinical scopes.", nil)
	}
	return nil
}
