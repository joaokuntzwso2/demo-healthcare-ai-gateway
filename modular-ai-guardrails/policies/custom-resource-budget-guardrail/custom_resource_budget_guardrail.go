...
const policyName = "custom-resource-budget-guardrail"
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    size:=len(req.Body.Content);maxTokens:=intField(v,"max_tokens");if size>262144||countMessages(v)>32||countTools(v)>16||maxTokens>4096{setFinding(req,"RESOURCE_BUDGET_EXCEEDED","Request exceeds the configured clinical AI resource budget.",map[string]interface{}{"bytes":size,"messages":countMessages(v),"tools":countTools(v),"maxTokens":maxTokens})}
    return nil
}
