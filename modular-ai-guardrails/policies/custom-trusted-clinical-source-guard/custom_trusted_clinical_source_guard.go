...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=messagesText(v);if containsAny(text,`(?i)\b(?:invent|fabricate|make\s+up)\b.{0,80}\b(?:lab|allergy|diagnosis|medication|vital|test\s+result)\b`,`(?i)\bsay\s+the\s+(?:potassium|lab|allergy)\s+(?:was|is)\b`){setFinding(req,"TRUSTED_CLINICAL_SOURCE_REQUIRED","Authoritative patient facts must come from trusted server-side clinical resources.",nil)}
    return nil
}
