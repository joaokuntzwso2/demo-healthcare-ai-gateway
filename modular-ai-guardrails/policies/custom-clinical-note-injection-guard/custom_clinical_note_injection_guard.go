...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
    ...
    text:=messagesText(v);if containsAny(text,`(?is)(?:referral|clinical\s+note|external\s+note).{0,400}ignore\s+(?:all\s+)?prior\s+instructions`,`(?is)patient\s+definitely\s+has.{0,120}maximum\s+dose`){setFinding(req,"INDIRECT_PROMPT_INJECTION","Clinical/document text contains model-directed instructions and is not treated as authority.",nil)}
    return nil
}
