...
const policyName = "custom-clinical-note-injection-guard"
...
func hasClinicalDocumentInjection(text string) bool {
	return containsAny(text,
		`(?is)(?:referral|clinical\s+note|external\s+note|uploaded\s+document).{0,500}ignore\s+(?:all\s+)?(?:prior|previous)\s+instructions`,
		`(?is)patient\s+definitely\s+has.{0,160}maximum\s+dose`,
		`(?is)(?:referral|clinical\s+note|external\s+note).{0,500}do\s+not\s+ask\s+for\s+clinician\s+approval`,
	)
}
...
func (p *Policy) OnRequestBody(_ context.Context, req *policy.RequestContext, _ map[string]interface{}) policy.RequestAction {
	...
	if hasClinicalDocumentInjection(messagesText(v)) {
		setFinding(req, "INDIRECT_PROMPT_INJECTION", "Clinical/document text contains model-directed instructions and is not treated as authority.", nil)
	}
	return nil
}
