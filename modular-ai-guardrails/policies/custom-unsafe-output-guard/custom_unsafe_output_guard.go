package customunsafeoutputguard
import (
    "context"
    "encoding/json"
    "fmt"
...
const policyName = "custom-unsafe-output-guard"
...
func (p *Policy) OnResponseBody(_ context.Context, resp *policy.ResponseContext, _ map[string]interface{}) policy.ResponseAction {text,err:=assistantText(resp);if err!=nil{if text==""{return nil};return blockResponse("RESPONSE_STRUCTURE_INVALID",err.Error(),nil)};if containsAny(text,`(?i)\bI\s+(?:have\s+)?(?:placed|submitted|executed)\s+(?:the\s+)?(?:prescription|medication|test|order)`,`(?i)\byou\s+should\s+(?:start|stop|double)\s+(?:the\s+)?(?:medication|dose)`,`(?i)\bdo\s+not\s+(?:contact|consult|seek)\s+(?:a\s+)?clinician`){return blockResponse("UNSAFE_CLINICAL_OUTPUT","Model output implies autonomous clinical action or unsafe directive.",nil)};return policy.DownstreamResponseModifications{}}
