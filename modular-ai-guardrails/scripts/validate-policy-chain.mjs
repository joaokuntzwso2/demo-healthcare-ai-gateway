...
for(const file of ['clinical-policy-chain.json','patient-support-policy-chain.json']){
 const chain=JSON.parse(await readFile(resolve(root,'config',file),'utf8')); const names=chain.map(x=>x.name);
 ...
 for(const [i,stage] of chain.entries()){
  for(const binding of (stage.paths||[])){
   if(i>0 && binding.path!=='/v1/chat/completions') throw new Error(`${file}: ${stage.name} must match banking-compatible /v1/chat/completions, got ${binding.path}`);
  }
 }
...
console.log('Policy-chain order and banking-compatible /v1 proxy binding validated.');
