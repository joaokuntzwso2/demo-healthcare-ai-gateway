...
async function req(url,{method='GET',headers={},body}={}){const r=await fetch(url,{method,headers:{Authorization:auth,Accept:'application/json',...headers},body});const text=await r.text();let data;try{data=text?JSON.parse(text):null}catch{data=text}return{status:r.status,ok:r.ok,data,text}}
const base=`${controller}/api/management/v1`;
for(const [id,file] of [['clinical-ai-secure','clinical-policy-chain.json'],['patient-support-ai-secure','patient-support-policy-chain.json']]){
  ...
  for(const stage of chain){for(const binding of (stage.paths||[])){if(stage.name!=='api-key-auth'&&binding.path!=='/chat/completions')throw new Error(`${file}: ${stage.name} has invalid proxy resource ${binding.path}; expected /chat/completions`)}}
  const obj=current.data;delete obj.status;obj.spec.policies=chain;
  ...
  console.log(`${id}: reapplied ${chain.length}-stage chain on /chat/completions`);
}
